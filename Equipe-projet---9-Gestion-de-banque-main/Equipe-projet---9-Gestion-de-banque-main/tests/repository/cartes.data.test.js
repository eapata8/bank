...
describe("findCartes", () => {
  it("retourne toutes les cartes pour un admin (canReadAll=true)", async () => {
    const rows = [{ id: 1, numero_compte: "**** 1111" }];
...
describe("findCarteById", () => {
  it("retourne la carte si trouvee", async () => {
    const carte = { id: 1, numero_compte: "**** 9999", type_carte: "VISA" };
...
describe("findAuthorizedCarteById", () => {
  it("retourne la carte si autorisee pour l'utilisateur", async () => {
    const carte = { id: 5, numero_compte: "**** 5555" };
    mockDb.query.mockResolvedValueOnce([[carte]]);
...
describe("createCarte", () => {
  it("insere une nouvelle carte et retourne le resultat", async () => {
    ...
    const result = await repo.createCarte({
      clientId: 1,
      numeroCompte: "4532 8814 7700 1234",
      cvv: "742",
      typeCarte: "VISA",
      limiteCredit: 5000,
    ...
    expect(mockDb.query).toHaveBeenCalledWith(
      ...
      [1, "4532 8814 7700 1234", "742", "VISA", 5000, "2030-12-31"]
    );
  });
});
...
describe("updateSoldeUtilise", () => {
  it("met à jour le solde utilisé de la carte", async () => {
    mockDb.query.mockResolvedValueOnce([{ affectedRows: 1 }]);
    await repo.updateSoldeUtilise(3, 500);
    expect(mockDb.query).toHaveBeenCalledWith(
      expect.stringContaining("solde_utilise = ?"),
      [500, 3]
    );
  });
});
